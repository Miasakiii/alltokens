...
export default function ModelBreakdown({ data, loading }: Props) {
  const zh = useLang().lang === 'zh';
  const [expanded, setExpanded] = useState(false);

  if (loading) {
    return (
      <Card title={zh ? '模型排行' : 'Model Breakdown'}>
        <LoadingRows rows={5} />
      </Card>
    );
  }
  ...
  if (!data || data.length === 0) {
    return (
      <Card title={zh ? '模型排行' : 'Model Breakdown'}>
        <EmptyState title={zh ? '暂无模型数据' : 'No model data yet'} />
      </Card>
    );
  }
  ...
  return (
    <Card
      title={zh ? '模型排行' : 'Model Breakdown'}
      subtitle={
        zh
          ? `${sorted.length} 个模型 · ${formatTokens(total)} tokens`
          : `${sorted.length} models · ${formatTokens(total)} tokens`
      }
    >
      <div className="space-y-3">
        {visible.map((d, i) => {
          ...
          return (
            <div key={`${d.provider}-${d.model}`} className="min-w-0">
        ...
        >
          {expanded
            ? zh
              ...
              : 'Show less'
            : zh
              ? `展开全部 ${sorted.length} 个模型 (+${hiddenCount})`
              : `Show all ${sorted.length} models (+${hiddenCount})`}
        </button>
      )}
    </Card>
  );
