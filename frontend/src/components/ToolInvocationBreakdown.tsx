import type { ToolInvocationStats } from '../api/types';
import { formatTokens } from '../utils/format';
import { useLang } from '../i18n';
import { Card, LoadingRows, EmptyState, Meter } from './ui/primitives';

interface Props {
  data: ToolInvocationStats[] | null;
  loading: boolean;
}

export default function ToolInvocationBreakdown({ data, loading }: Props) {
  const zh = useLang().lang === 'zh';

  if (loading || !data || data.length === 0) {
    return (
      <Card
        title={zh ? 'Tool 调用 TOP' : 'Top Tools'}
        subtitle={zh ? '按调用次数排序' : 'Sorted by invocation count'}
      >
        {loading ? (
          <LoadingRows rows={5} />
        ) : (
          <EmptyState title={zh ? '暂无 Tool 调用数据' : 'No tool invocation data yet'} />
        )}
      </Card>
    );
  }
    ...
    >
      ...
      {sorted.length > 10 && (
        <p className="text-xs text-faint mt-3">
          {zh ? `另有 ${sorted.length - 10} 个 Tool` : `+${sorted.length - 10} more tools`}
        </p>
      )}
    </Card>
  );
